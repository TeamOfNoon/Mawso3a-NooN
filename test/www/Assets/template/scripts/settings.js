...
function saveSetting(name, value, bPersistent)
{
    ...
    // COOKIE SECOND
    else if(isCookieFullySupported() || gHost == null)
    {
...
function isLocalDBSupported()
{
	if(gbLocalStorageSupported === null) {
		// Check if localStorage exists in the window object
		if(typeof window.localStorage != 'undefined') {
			try {
				window.localStorage.setItem('dummyTestForLocalStorage', 1);
				window.localStorage.removeItem('dummyTestForLocalStorage');
