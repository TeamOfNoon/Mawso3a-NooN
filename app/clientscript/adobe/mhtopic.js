...
function GetSearchRhhltermFromURL(bRemoveSlash)

{
	var strQuery = getUrlParameter(RHHLTERM);
...
function initTopicLoad()
{
        ...
        hit_id =  getParameterByName("hit",  window.location.href)
        //addRhLoadCompleteEvent(StartHighLightSearch);
	

setTimeout(function () {
if(location.href.split('p=')[1]){  
postid = getParameterByName("p",  window.location.href);
...
function StartHighLightSearch(do_hit)
{


         highlight = read_Setting("hit_true", "true", "all");
         part_hit = read_Setting("part_match", "false", "all");


	//var strTerms = GetSearchTextFromURL();
	//var strTerms = getUrlParameter(RHHLTERM);

   
       var part_hit_check = (part_hit == "true");
      if(part_match != null){
       if(part_hit_check != part_match ){
       //thrers change apply highlight
	...
	var strTerms =	GetSearchRhhltermFromURL();
	...
	if(highlight == "true"  && (window.location != window.parent.location)) {
	...
	if (strTerms[0] == "\"" && strTerms[strTerms.length - 1] == "\"") {

	
	 strTerms = strTerms.replace(/^"(.+(?="$))"$/, '$1');
        strTerms = createAccentRegexp(strTerms).split(' ').join('(<[^>]+>|[^\\p{L}\\p{N}])*');
	...
	remove_repeated = strTerms.split(' ').filter(function(item, i, allItems) {
	...
	if(part_hit == "true") {
	   ...
	   pattern = XRegExp("(" + strTerms + ")", "gi");
        ...
        if (supportsLookbehind() == true) {
            ...
            pattern = XRegExp("(^|[^ً\\p{L}\\p{N}\\p{Mn}\u200C]|[ۚۗۖۙ])(" + strTerms + ")(?=[^ً\\p{L}\\p{N}\\p{Mn}\u200C]|[ۚۗۖۙ]|$)", "gi");
        } else {
            pattern = XRegExp("(^|[^ً\\p{L}\\p{N}\\p{Mn}\u200C]|[ۚۗۖۙ])(" + strTerms + ")(?=[^ً\\p{L}\\p{N}\\p{Mn}\u200C]|[ۚۗۖۙ]|$)", "gi");
        }
	...
	//alert(part_hit+part_match);
         ...
         if(apply_mark == true){
               ...
               $('em').contents().unwrap();
                    $( 'em:empty' ).remove();	
             if(postid){
				apply_highlight($('#pagebody  #postid_'+postid).get( 0 ),pattern,null,null); 
			 }
			 else{
				apply_highlight($('#pagebody').get( 0 ),pattern,null,null); 
			 }
...
seamless.polyfill(); 
//seamless.windowScrollByPolyfill
seamless.elementScrollIntoView($("#hit_"+hit_id)[0], {
...
seamless.polyfill(); 
seamless.elementScrollIntoView($("em").get(0), {
       block: 'center', inline: 'center'
    });	
...
setTimeout(function () {
 $.postMessage(
	...
	$.postMessage(
        '{"eventname":"send_page_info", "url":"'+utf8_to_b64( encodeURIComponent(window.location.href.toString()) )+'", "title":"'+utf8_to_b64( document.title )+'"}', // The message to send (string)
        "*", // The host of the target window (i.e. http://www.vistaprint.com)
        parent // A reference to the target window
      );
...
setTimeout(function () {
    ...
    scrolling = true;	
            ...
            if (window.location != window.parent.location) {
               	 ...
               	 $.postMessage(
        '{"eventname":"currPATH"}', // The message to send (string)
        "*", // The host of the target window (i.e. http://www.vistaprint.com)
        parent // A reference to the target window
     ); 
                ...
                save_Setting("currPATH", window.parent.location.toString().split('app/').pop(), "all");
...
function callbackHighlightTxtColorRead()
{
