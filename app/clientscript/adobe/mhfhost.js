...
gSearchString = search || GetSearchTextFromURL();
...
if (document.getElementById("checkbox_id5").checked) {
is_snipit_enabled = true;
}
else{
...
if (document.getElementById("checkbox_id6").checked) {


save_Setting('part_match', "true", 'all');

}
else{


save_Setting('part_match', "false", 'all');

}


toggle_settings("hide");
$("#page_num").hide();
$("#end_ResList").hide();
...
_a_QueryResult =null;
	   ...
	   //setTimeout(function(){  fixImage(document.getElementById("clear")); }, 100);
...
function callbackAndSearchFlagRead(andFlag)
{
	...
	$("#searchBox").addClass("input-with-value");
...
if(topic_info[i]["postid"] == queryId){

        a_this.strTopicInfo = topic_info[i]["rd"];
		a_this.strTopicContext = topic_info[i]["ct"];
}
}
}
else{
        topicInfo = theXmlReader.getTopicRec(queryId);
...
function HuginHunter()
{
	...
	{
		if ( a_strRecord == "" || a_strRecord == null)
			return;
...
function ftsContextLoaded(ftsProjDirArr)
{
	...
	striped_Normlized = strip_And_Normlize(gSearchString);
	
	
	
if (search_type != "exact") {
if (document.getElementById("checkbox_id4").disabled == false) {

if (document.getElementById("checkbox_id4").checked == false) {

striped_Normlized = remove_stopWrods(striped_Normlized);
}
}
}
...
if (last_ajax && last_ajax.readyState !== 4) {
    last_ajax.abort();
}
var is404 = false;
...
xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4) {
        if (xmlhttp.status == 200) {
            try {
                get_recored = true;
                strRecord_d = JSON.parse(xmlhttp.responseText)[0];
                topic_info = JSON.parse(xmlhttp.responseText)[1];

                goOdinHunter.strQuery = striped_Normlized;
                Query();
            } catch (error) {
...
// Prepare the GET request with proper encoding
var url = "admin/get_rec.php?q=" + encodeURIComponent(striped_Normlized.replace(/['"]+/g, '').trim());

// Open the connection, set the request header, and send the request
xmlhttp.open("GET", url, true);
xmlhttp.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
xmlhttp.send();

// Store the last XMLHttpRequest object for potential abortion
last_ajax = xmlhttp;
	...
	if (g_CurPage < 1)
	    g_CurPage = 1 ;
	        ...
	        if (search_type == "exact") {
		                ...
		                striped_Normlized = '"' + striped_Normlized.replace(/["']/g, "") + '"';
	        ...
	        var strParams = "?" + RHHLTERM + "=" +  encodeURIComponent( striped_Normlized ) + "&" + RHSYNSTR + "=" + encodeURIComponent(gstrSyn);
...
timeoutId[abort_cont+1] = setTimeout(function(){
 ...
 _bShowAll= bShowAll
 _a_QueryResult = a_QueryResult
 _strParams  = strParams;
 setTimeout(function(){
  
lunch_vlist2(a_QueryResult, strParams, "searchResList", "contentholder", i, g_nMaxResult, g_CurPage, bShowAll,true);
...
window.clearTimeout(timeoutId[abort_cont]);  
abort_cont++;
...
for( ; (i < a_QueryResult.aTopics.length); i++ )
		{
			...
			sLine += writeResult( count,szTopicURL,
...
function secondFunction(a_nPageNumber){
