...
function initInputTextBoxes()
{
	...
	for(i=0; i<len; i++)
	{
		...
		if(typeof is_ios_app != 'undefined') {   
          var obj = {event: "search_input", arg: searchText};
		...
		if(gbIE5 && !gbIE10)
		{
			...
			{
				var input = inputs[i];
				input.addEventListener('focus', onTextBoxFocus, { passive: true});
				input.addEventListener('blur', onTextBoxBlur, { passive: true});
			}
		}
	}
}
...
function patchInputForSubmit(input, func)
{
	...
	if(formElem.addEventListener)
		formElem.addEventListener('submit', func, { passive: true});
...
var gProjDataFile = rootpath+"clientscript/adobe/projectdata.js";
